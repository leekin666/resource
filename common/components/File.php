...
class File extends Component
{
    ...
    public function saveToOss($allow = NULL, $host = NULL){
        # token 验证
        if($this->token != NULL){
            if(!isset($_POST['token']) || $_POST['token'] != $this->token){
                return -5;
        ...
        if(!empty($_FILES) && isset($_FILES[$this->frm_name])){
            ...
            if(count($filelist) === 1){
                ...
                $ext = $this->get_file_ext($filelist[0]['name']); # 文件后缀名
                $fileName = $ext == '' ? $this->uuid() : $this->uuid().'.'.$ext;
                $ossRes = Yii::$app->Aliyunoss->upload($fileName,$filelist[0]['tmp_name']);
                if(isset($ossRes) && $ossRes['code'] == 0) {
                    # - 上传文件回调数据信息
                    $tmp_arr['name'] = $filelist[0]['name'];    # 文件上传时的原名称
                    $tmp_arr['ext'] = $ext;                     # 文件后缀名
                    $tmp_arr['mime'] = $filelist[0]['type'];    # 文件MIME
                    $tmp_arr['size'] = $filelist[0]['size'];    # 文件大小(单位:字节)
                    $tmp_arr['savename'] = $fileName;           # 文件保存在服务器上名称
                    $tmp_arr['url'] = $ossRes['url'];           # 文件访问URL地址
                    $new_arr[] = $tmp_arr;
                }
            }else{
                # 多文件上传
