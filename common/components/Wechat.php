...
class Wechat extends Component
{
     ...
     * 根据openid列表群发
     */
    const WECHAT_ARITICLES_OPENIDS_SEND_URL = '/cgi-bin/message/mass/send?';
     ...
     * 批量获取用户信息
     */
    const WECHAT_MEMBER_INFO_BATCH_URL = '/cgi-bin/user/info/batchget?';
     ...
     * 获取自定义菜单配置接口
     */
    const WECHAT_CURRENT_SELF_MENU_INFO = '/cgi-bin/get_current_selfmenu_info?';
    ...
    {
        $this->appId = Yii::$app->params['wechatTest']['appId'];
        $this->appSecret = Yii::$app->params['wechatTest']['appSecret'];
        $this->token = Yii::$app->params['wechatTest']['token'];
        parent::__construct();
    }
    ...
    public function transmitText($request_data,$content){
        $textTpl = "<xml> 
       ...
       <Content><![CDATA[%s]]></Content> 
        ...
        $result = sprintf($textTpl, $request_data['FromUserName'], $request_data['ToUserName'], time(), $content);
        return $result;
    }
...
//    public function getAccessToken($force = false)
//    {
//        $token_info = $this->getCache('xiaozhan_beikao_access_token', false);
//        if ($force || !$token_info || ($token_info['expire'] < YII_BEGIN_TIME)) {
//            if (!($token_info = $this->requestAccessToken())) {
//                throw new HttpException(500, 'Fail to get access_token from wechat server.');
//            }
//            $this->trigger(self::EVENT_AFTER_ACCESS_TOKEN_UPDATE, new Event(['data' => $token_info]));
//            $this->setCache('xiaozhan_beikao_access_token', $token_info, ($token_info['expires_in'] - 10));
//        }
//
...
//    }
    ...
    public function createMenu(array $buttons)
    {
        ...
        echo json_encode($result, 256).PHP_EOL;
    ...
    {
        $result = $this->httpGet(self::WECHAT_CURRENT_SELF_MENU_INFO . 'access_token=' . $this->getAccessToken());
        return $result;
    }
     ...
     * @return int|bool
     */
    public function sendTemplateMessage($toUser, $templateId, array $data, $clickUrl = null)
    {
        $data = [
                'touser' => $toUser,
    ...
    public function sendArticlesByOpenids(array $openid_list, $content, $type)
    {
        if ($type !== 'video') {
            $content = [
                $type === 'text' ? 'content' : 'media_id' => $content
        ...
        $result = $this->httpRaw(self::WECHAT_ARITICLES_OPENIDS_SEND_URL . 'access_token=' . $this->getAccessToken(), [
            'touser' => $openid_list,
            $type => $content,
            'msgtype' => $type
        ]);
     ...
     * @return array|bool
     */
    public function getMemberInfoBatch($openId_list)
    {
        Yii::info("openid_list->".json_encode($openId_list, JSON_UNESCAPED_UNICODE));
        $result = $this->httpRaw(self::WECHAT_MEMBER_INFO_BATCH_URL . 'access_token=' . $this->getAccessToken(), [
            'user_list' => $openId_list,
        ]);
    ...
    protected function setCache($name, $value, $duration = null)
    {
        ...
        return Yii::$app->redis->setex($name , $duration-10, $value);
    }

//    protected function setCache($name, $value, $duration = null)
//    {
//        $duration === null && $duration = $this->cacheTime;
//        return Yii::$app->redis->set($this->getCacheKey($name), json_encode($value), $duration);
//    }
    ...
    protected function http($url, $params = null, $type = 'get')
    {
        ...
        switch ($type) {
            ...
            case 'post':
                ...
                Yii::info("params msg.->".json_encode($params));
